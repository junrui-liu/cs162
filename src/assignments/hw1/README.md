...
> Written problems due: **Wednesday, June 25, 2025, before lecture**
> 
...
1. **Written problems** are marked with ✍️.
   - Please turn in a physical copy of your solutions before the lecture start time on the due date.
   ...
   - On the first page of your solutions, please include your name, student ID, and a table containing the self-grades for each sub-problem, like this:
...
- Use of generative AI tools and coding assistants is not allowed for this assignment. You will be able to use them for future assignments (maybe), but not for this one, espcially if you're new to Python.

- If you have any questions about the assignment, please come to the office hours, or post them in the `#hw1` Discord channel.
...
Your task is to implement the `eval` function that evaluates the input expression given an "user input" for `x`, using the following signature:
```python
...
Define a function `remove_compose` that takes an expression and returns an equivalent expression that does not use the composition operator `|>`.
```python
...
$$
where $P$ is a proposition, $b$ is a boolean value ($\textsf{True}$ or $\textsf{False}$), and $\alpha$ is an assignment (a partial function) from propositional variables to boolean values. Intuitively, this judgment means that under assignment $\alpha$, proposition $P$ evaluates to boolean value $b$.
...
### Problem 3-B (✍️, ⭐️extra credit⭐️, 2 points)
